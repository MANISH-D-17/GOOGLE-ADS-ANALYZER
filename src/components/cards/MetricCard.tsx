...
interface MetricCardProps {
  label?: string; // mapping from title
  title?: string;
  value: string | number;
  change?: string; // mapping from trend
  trend?: number;
  positive?: boolean;
  color?: string;
...
export const MetricCard: React.FC<MetricCardProps> = ({ 
  ...
  const isPositive = positive !== undefined ? positive : (trend !== undefined ? trend > 0 : true);
  const displayChange = change || (trend !== undefined ? `${Math.abs(trend)}%` : '');
      ...
      <div className="relative z-10">
        <div className="flex items-center gap-2 mb-4">
          <div className="text-gray-400 group-hover:text-indigo-600 transition-all duration-300">
            {React.isValidElement(icon) ? React.cloneElement(icon as React.ReactElement, { size: 14, strokeWidth: 3 }) : icon}
          </div>
          <div className="text-[10px] font-black text-gray-400 uppercase tracking-widest">{displayLabel}</div>
        </div>
        
        {isLoading ? (
          <div className="h-8 bg-gray-100 rounded animate-pulse w-1/2 mb-2"></div>
        ) : (
          <div className="text-3xl font-black text-gray-900 mb-2 tracking-tight">{displayValue}</div>
        )}
        
        {subtitle ? (
          <div className="text-[11px] text-gray-500 font-bold">{subtitle}</div>
        ) : (
          displayChange && !isLoading && (
            <div className={cn(
