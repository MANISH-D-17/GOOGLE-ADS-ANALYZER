...
const SummaryCards: React.FC<SummaryCardsProps> = ({ skus }) => {
  // Calculate totals
  const totalSpend = skus.reduce((acc, s) => acc + s.spend, 0);
  const totalRevenue = skus.reduce((acc, s) => acc + s.revenue, 0);
  const totalConversions = skus.reduce((acc, s) => acc + s.conversions, 0);
  const blendedRoas = totalRevenue / (totalSpend || 1);
  ...
  const kpis = [
    { label: 'Spend', value: formatRupees(totalSpend), change: '+6.2%', positive: true, color: '#3b82f6', icon: <CreditCard /> },
    { label: 'Revenue', value: formatRupees(totalRevenue), change: '+14.8%', positive: true, color: '#10b981', icon: <IndianRupee /> },
    { label: 'Blended ROAS', value: `${blendedRoas.toFixed(2)}x`, change: '+8.1%', positive: true, color: '#8b5cf6', icon: <Zap /> },
    { label: 'Conversions', value: totalConversions.toLocaleString(), change: '+11.4%', positive: true, color: '#10b981', icon: <ShoppingBag /> },
  ];
