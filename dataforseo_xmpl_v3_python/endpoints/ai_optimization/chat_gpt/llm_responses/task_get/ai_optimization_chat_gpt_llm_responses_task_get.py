...
try:
    ...
    tasks_ready = client.get('/v3/ai_optimization/chat_gpt/llm_responses/tasks_ready')
        ...
        for task in tasks_ready['tasks']:
            if 'result' in task and task['result']:
                for task_ready in task['result']:
                    ...
                    # GET /v3/ai_optimization/chat_gpt/llm_responses/task_get
                    if 'endpoint' in task_ready:
                        result.append(client.get(task_ready['endpoint']))
                    
                    # #3 - another way to get the task results by id
                    # GET /v3/ai_optimization/chat_gpt/llm_responses/task_get
                    """
                    if 'id' in task_ready:
                        result.append(client.get('/v3/ai_optimization/chat_gpt/llm_responses/task_get/' + task_ready['id']))
                    """
    print(result)
